...
_Infraestrutura e performance:_
...
_Segurança:_ 4. Headers HTTP de segurança em `next.config.ts`: `Content-Security-Policy`, `X-Frame-Options`, `Strict-Transport-Security`, `X-Content-Type-Options`, `Referrer-Policy`. 5. Auditoria sistemática de IDOR: revisar todos os server actions e route handlers — confirmar que toda operação de leitura e escrita valida `userId` da sessão antes de acessar o banco. 6. Rate limiting no endpoint de autenticação (Upstash Ratelimit ou middleware Vercel) para proteção contra brute force. 7. Confirmar que `SUPABASE_SECRET_KEY` nunca é referenciada em código de client component nem em variável com prefixo `NEXT_PUBLIC_`.

_LGPD e compliance:_ 8. Feature de exclusão de conta e dados (direito ao esquecimento): server action que deleta em cascata todos os dados do usuário — procedimentos, materiais, configurações, snapshots — e então remove o registro `User` e o usuário no Supabase Auth. 9. Cookie consent banner obrigatório antes de ativar cookies de terceiros (necessário para Posthog). Deve persistir preferência do usuário. 10. Documentar no schema Prisma quais campos de `User` são dados pessoais e por quanto tempo são retidos após cancelamento/exclusão.

_Observabilidade:_ 11. Setup Sentry (`@sentry/nextjs`): captura de exceções em server actions e route handlers com contexto de usuário (sem PII sensível). 12. Instalar `@vercel/analytics` e adicionar `<Analytics />` no `app/layout.tsx` para rastrear page views e eventos de navegação. 13. Instalar `@vercel/speed-insights` e adicionar `<SpeedInsights />` no `app/layout.tsx` para coletar Core Web Vitals (LCP, CLS, INP) por rota em produção. 14. Setup Posthog: identificar usuário após login (`posthog.identify`), capturar eventos em ações-chave — procedimento criado, custo salvo, PDF exportado, simulador usado, snapshot gerado.

**Dependências:** Nenhuma (pode rodar imediatamente após Fase 5).

**Critério de "feito":** Headers de segurança verificáveis via securityheaders.com com nota ≥ B; Sentry recebendo eventos de erro de teste; Vercel Analytics e Speed Insights visíveis no dashboard do projeto Vercel após deploy; Posthog registrando sessão e eventos de usuário de teste; feature de exclusão de conta remove todos os dados em cascata; cookie consent funcional.
...
3. **Seção "Em breve"** como página `/em-breve` ou seção do dashboard:
   - Cards visuais com os itens da Fase 11 (nice-to-haves do produto).
...
_Schema Prisma — novos campos em `User`:_ 3. Migration adicionando:
...
_Fluxo de cadastro com trial:_ 4. Ao criar conta: criar customer no Stripe → criar subscription com `trial_period_days: 14` e coleta de cartão via Stripe Checkout (modo subscription) ou Stripe Elements. 5. Middleware de proteção por status de assinatura: redirecionar para `/assinar` se `subscriptionStatus` não for `trialing` ou `active`. 6. Página `/assinar` para usuários com trial expirado, cancelado ou sem assinatura: exibe pricing, tabela mensal/anual e CTA de reativação. 7. Banner de aviso no dashboard durante o trial exibindo dias restantes e link para gerenciar assinatura.

_Webhooks:_ 8. Route handler `POST /api/stripe/webhook` com verificação de assinatura HMAC:
...
_Área do usuário (`/conta`):_ 9. Página `/conta` acessível pela sidebar (novo item de navegação):
...
**Critério de "feito":** Usuário consegue se cadastrar com cartão, usar o app por 14 dias em trial, ser cobrado automaticamente ao fim do trial, e gerenciar/cancelar a assinatura pelo portal Stripe. Webhook atualiza `subscriptionStatus` em tempo real. Acesso bloqueado para assinaturas expiradas. Pedido de registro de marca protocolado no INPI com número de protocolo em mãos.

---

## Fase 9 — Experiência Mobile

**Objetivo:** Garantir que o produto funciona com qualidade em smartphones. Dentistas usam o celular entre consultas para verificar margens e preços — uma tela quebrada ou inutilizável nesse contexto é abandono imediato antes mesmo de o usuário sentir o valor.

**Esforço estimado:** 3–5 dias

**Entregáveis:**

_Navegação e layout global:_

1. Substituir a sidebar desktop por um menu hambúrguer com drawer deslizante em viewports < 768px. Sidebar atual ocupa espaço útil e não colapsa adequadamente.
2. Revisar padding e margens de todos os layouts de página (`(dashboard)/`) para evitar overflow horizontal em telas de 375–430px (iPhone SE → iPhone 16 Pro Max).
3. Garantir que o header do dashboard e o título de cada página não ultrapassam a largura da tela nem colidem com ícones de ação.

_Listagens e tabelas:_ 4. Tabela de procedimentos: em mobile, colapsar colunas secundárias (código VRPO, duração) e exibir nome + badge de margem + preço em layout de card empilhado (ou tabela com scroll horizontal controlado). 5. Tabela do comparativo VRPO: idem — colunas de diferença percentual colapsadas; foco em nome do procedimento, preço atual e referência. 6. Histórico de snapshots: cards responsivos em vez de tabela quando < 640px.

_Formulários:_ 7. Formulário de Custos Fixos: inputs numéricos com `inputMode="decimal"` para acionar teclado numérico no iOS/Android; campos de percentual com sufixo "%" visível mesmo em largura reduzida. 8. Formulário de procedimentos e materiais: `inputMode="decimal"` nos campos de preço e consumo; botões de ação (salvar, cancelar) fixados no rodapé da tela em mobile para não exigir scroll até o final. 9. Wizard de onboarding: testar cada passo em viewport 375px — botões "Próximo" e "Voltar" devem ser acessíveis sem scroll desnecessário.

_Simulador e dashboard:_ 10. Simulador (`/simulador`): painel de controles que hoje fica em sidebar lateral deve migrar para acordeão ou sheet deslizante do rodapé em mobile; lista de procedimentos impactados deve ser legível sem scroll horizontal. 11. Dashboard: revisar grid de cards para 1 coluna em mobile; garantir que o card "Faturamento mínimo semanal" e a seção "Atenção necessária" não são cortados.

_Qualidade e testes:_ 12. Auditoria de touch targets: todos os botões, links e itens de lista devem ter área de toque mínima de 44 × 44px (diretriz Apple/Google). 13. Testes em dispositivos reais ou emulados: iOS Safari (iPhone SE 375px, iPhone 14 390px) e Android Chrome (Samsung Galaxy S23 360px). Registrar e corrigir quebras de layout. 14. Lighthouse Mobile no Vercel preview: Performance ≥ 75, sem erros de acessibilidade críticos em nenhuma tela principal.

**Dependências:** Fase 8 (as páginas de trial, assinatura e `/conta` também precisam ser responsivas — só faz sentido auditar o produto completo).

**Critério de "feito":** Todas as telas principais (dashboard, procedimentos, custos fixos, simulador, onboarding, `/conta`, `/assinar`) navegáveis e utilizáveis em viewport 375px sem overflow horizontal e sem touch targets inacessíveis. Lighthouse Mobile ≥ 75 no Vercel preview.
...
_Documentos legais e propriedade intelectual:_ 4. Página `/termos-de-uso`: Termos de Uso para SaaS brasileiro — condições do trial, cobrança automática, cancelamento, limitação de responsabilidade, lei aplicável (Brasil). 5. Página `/politica-de-privacidade`: Política de Privacidade em conformidade com LGPD — dados coletados, finalidade, retenção, direitos do titular (acesso, correção, exclusão, portabilidade), contato do responsável pelo tratamento. 6. Checkbox de aceite nos formulários de cadastro e login: "Li e aceito os [Termos de Uso] e a [Política de Privacidade]". 7. Protocolar **Registro de Programa de Computador** no INPI (Lei 9.609/98) com o código-fonte da versão de lançamento. Fazer nesta fase, quando o produto está estável e a versão é representativa. O processo leva 3–6 meses e gera certificado de titularidade sobre o software. Depositar o hash SHA-256 do repositório junto à documentação interna para evidência de anterioridade.

_Marketing:_ 8. Arquivo `MARKETING.md` na raiz do projeto com estratégia completa de lançamento:
...
**Dependências:** Fases 8 e 9 (o CTA da landing page aponta para o cadastro com Stripe; os screenshots do app devem refletir a experiência mobile já polida; os Termos devem referenciar as condições reais do trial e do plano de preços).

**Critério de "feito":** Landing page no ar em produção com Lighthouse Performance ≥ 85; Termos de Uso e Política de Privacidade publicados e linkados no rodapé, no cadastro e no login; pedido de Registro de Programa de Computador protocolado no INPI; `MARKETING.md` revisado e aprovado pelo responsável pelo produto.

---

## Fase 11 — Nice-to-haves de Produto (sob demanda)
...
| 9    | Experiência mobile                   | 3–5d     | Pendente | 8              |
| 10   | Landing page + legal + marketing     | 4–6d     | Pendente | 8+9            |
| 11   | Nice-to-haves de produto             | Variável | Backlog  | Qualquer ordem |
