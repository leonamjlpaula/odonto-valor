 ...
 */
export const calcularCustoFixoPorMinuto = cache((userId: string): Promise<number> =>
  unstable_cache(
    async () => {
...
export const getPercConfig = cache((userId: string): Promise<{ percImpostos: number; percTaxaCartao: number }> =>
  unstable_cache(
    async () => {
      const config = await prisma.custoFixoConfig.findUnique({
        where: { userId },
