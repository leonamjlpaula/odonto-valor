...
function parseVrpo(rows: Record<string, unknown>[]): {
  ...
  rows.forEach((row, i) => {
    ...
    if (valor === null || valor <= 0) {
      errors.push(
        `Linha ${lineNum}: valor_referencia inválido: "${valorRaw}" para código "${codigo}"`
      );
      return;
    }
...
function main() {
  ...
  const shouldRun = (target: string) => mode === 'todos' || mode === target;
  ...
  if (shouldRun('procedimentos')) {
    ...
    if (!sheet) {
      ...
      if (vrpoFromProcs.length > 0) {
        ...
        console.log(
          `  → ${vrpoFromProcs.length} referências VRPO extraídas da coluna vrpo_referencia`
        );
      }
    }
  }
